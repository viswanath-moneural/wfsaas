...
create index if not exists ix_parties_tenant_parent
  on public.parties(tenant_id, parent_party_id);

create index if not exists ix_parties_tenant_name
  on public.parties(tenant_id, lower(party_name));

create unique index if not exists uq_parties_tenant_code
  on public.parties(tenant_id, lower(party_code));

create unique index if not exists uq_parties_tenant_gst_nonnull
  on public.parties(tenant_id, lower(gst_number))
...
create index if not exists ix_contact_persons_tenant_party
  on public.contact_persons(tenant_id, party_id);
...
-- F) Backfill helpers
-- =========================================================
-- 1) Backfill tenant_id on contact_persons from party
...
insert into public.parties (
  id, tenant_id, party_code, party_name, legal_name,
  gst_number, phone, address, city, state, is_active,
  created_at, created_by, last_modified_at, last_modified_by
)
select
  gen_random_uuid(), c.tenant_id,
...
-- select count(*) from public.contact_persons where tenant_id is null;

create unique index if not exists uq_customers_tenant_code
  on public.customers(tenant_id, lower(customer_code));
create unique index if not exists uq_customers_tenant_party
  on public.customers(tenant_id, party_id)
  where party_id is not null;

create unique index if not exists uq_vendors_tenant_code
  on public.vendors(tenant_id, lower(vendor_code));
create unique index if not exists uq_vendors_tenant_party
  on public.vendors(tenant_id, party_id)
  where party_id is not null;
...
--    - optionally add NOT NULL on customers.party_id, vendors.party_id, contact_persons.tenant_id
