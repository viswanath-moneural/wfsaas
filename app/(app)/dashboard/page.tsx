...
export default function DashboardPage() {
  const { org, businessUnit } = useAuth()
  ...
  async function load(businessUnitId: string) {
    ...
    const [so, inv, po, grn, stock, move, cp, vp] = await Promise.all([
      supabase.from('sales_orders').select('id', { count: 'exact', head: true }).eq('business_unit_id', businessUnitId).in('status', ['draft', 'confirmed', 'dispatched']),
      supabase.from('invoices').select('id', { count: 'exact', head: true }).eq('business_unit_id', businessUnitId).not('status', 'in', '(paid,cancelled)'),
      supabase.from('purchase_orders').select('id', { count: 'exact', head: true }).eq('business_unit_id', businessUnitId).in('status', ['draft', 'approved']),
      supabase.from('goods_receipt_notes').select('id', { count: 'exact', head: true }).eq('business_unit_id', businessUnitId).eq('quality_status', 'pending'),
      supabase.from('stock_levels').select('current_stock, material_name').eq('business_unit_id', businessUnitId),
      supabase.from('stock_movements').select('id, movement_date, movement_type, qty, ref_table').eq('business_unit_id', businessUnitId).order('created_at', { ascending: false }).limit(8),
      supabase.from('customer_payments').select('id, payment_date, amount_paid, customers(customer_name)').eq('business_unit_id', businessUnitId).gte('payment_date', today),
      supabase.from('vendor_payments').select('id, payment_date, amount, vendors(vendor_name)').eq('business_unit_id', businessUnitId).gte('payment_date', today),
    ])
  ...
  if (!businessUnit) return <PageHeader title="Dashboard" description="Select a businessUnit to load ERP metrics." />

  return <>
    <PageHeader title="Dashboard" description={`${org?.name ?? 'Organisation'} / ${businessUnit.name}`} />
