...
export default function OpportunitiesPage() {
  ...
  async function load(businessUnitId: string) {
    ...
    const [{ data: oppData, error: oppError }, { data: leadData }, { data: customerData }] = await Promise.all([
      supabase.from('opportunities').select('id, opp_code, title, stage, expected_value, probability_pct, leads(name), customers(id, customer_code, customer_name, parties(party_name), contact_roles(role_type, is_primary, contact_persons(name, phone, email)))').eq('business_unit_id', businessUnitId).order('created_at', { ascending: false }),
      supabase.from('leads').select('id, lead_code, name').eq('business_unit_id', businessUnitId).order('created_at', { ascending: false }),
  ...
  if (!businessUnit) return <BusinessUnitSetupNotice title="Opportunities" description="Select or create a businessUnit before creating opportunities." />
  ...
  return <>
    <PageHeader title="Opportunities" description={`Opportunity pipeline for ${businessUnit.name}.`} />
