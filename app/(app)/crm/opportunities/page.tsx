...
export default function OpportunitiesPage() {
  ...
  const [form, setForm] = useState({ opp_code: '', lead_id: '', title: '', stage: 'prospecting', expected_value: '0', probability_pct: '10' })

  useEffect(() => { if (!tenant?.id) { setLoading(false); return } ; void load(tenant.id) }, [tenant?.id])
  async function load(tenantId: string) {
    const supabase = getSupabaseClient()
    const [{ data: oppData, error: oppError }, { data: leadData }] = await Promise.all([
      supabase.from('opportunities').select('id, opp_code, title, stage, expected_value, probability_pct, leads(name)').eq('tenant_id', tenantId).order('created_at', { ascending: false }),
  ...
  const columns: Column<any>[] = useMemo(() => [
    { key: 'opp_code', header: 'Opp Code' }, { key: 'title', header: 'Title' }, { key: 'leads', header: 'Lead', render: (_v, r) => r.leads?.name ?? '-' }, { key: 'stage', header: 'Stage', render: (v) => <Badge variant="info">{v || '-'}</Badge> }, { key: 'expected_value', header: 'Value', align: 'right' }, { key: 'probability_pct', header: 'Prob %', align: 'right' },
  ], [])
  return <>
    ...
    <section className="layout">
      <Card><h2>Add Opportunity</h2><form onSubmit={create}>
        ...
        <Button type="submit" loading={saving} disabled={!canCreate} fullWidth>Add opportunity</Button>
      </form></Card>
