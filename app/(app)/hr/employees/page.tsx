...
export default function EmployeesPage() {
  ...
  async function load(businessUnitId: string) {
    ...
    const { data, error: fetchError } = await supabase.from('employees').select('id, employee_code, full_name, phone, designation, department, date_of_joining, is_active').eq('business_unit_id', businessUnitId).order('created_at', { ascending: false })
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    const supabase = getSupabaseClient()
    const { data, error } = await supabase.from('employees').insert({ business_unit_id: businessUnit.id, employee_code: form.employee_code.trim(), full_name: form.full_name.trim(), phone: form.phone.trim() || null, designation: form.designation.trim() || null, department: form.department.trim() || null, date_of_joining: form.date_of_joining, is_active: true }).select('id').single()
    setSaving(false)
  ...
  if (!businessUnit) return <BusinessUnitSetupNotice title="Employees" description="Select or create a businessUnit before creating employees." />
  ...
  return <>
    <PageHeader title="Employees" description={`Employee records for ${businessUnit.name}.`} />
    <section className="layout">
      <Card><h2>Add Employee</h2><form onSubmit={create}>
        ...
        <Button title={!canCreate ? 'You do not have permission to create records.' : undefined} type="submit" loading={saving} disabled={!canCreate} fullWidth>Add employee</Button>
      </form></Card>
